...
devs::TimeType devs::component::RuleE::ta()
{
  if (priority_event_queue.empty())
    return TIME_MAX;
  return Time::distance(priority_event_queue.top().schedule_time);
}
  ...
  for (auto&x : xb) 
  {
    if (x.port == this->port_self_recv_at) {
      ...
      {  //不存在
        ...
        priority_event_queue.push(_Event(Time::now(), track_number));
      }
    }
...
void devs::component::RuleE::output_func(IO_Bag & yb)
{
  ...
  yb.insert(IO_Type(
    ...
    util::CreateSptrBlob(
      msg::JointMsg7I(e.track_name.c_str(), parent.name.c_str(), 0, Time::now())
    )
  ));
}
